...
int main() {
    Mat img = imread(path_image, IMREAD_COLOR);
    glob(png_dir, fn, true);

    if (fn.size() < 1) {
        cerr << "Can't find png in foulder" << endl;
        return EXIT_FAILURE;
    }
    if (img.empty()) {
...
void find_symbol_center(Point ret[2]) {
    ...
    cout << "Point dx" << ret[0] << " sx" << ret[1] << endl;
}
